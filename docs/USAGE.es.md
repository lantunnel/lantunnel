...
9. [En un router (OpenWrt)](#en-un-router-openwrt)
10. [Móviles](#móviles)
11. [Referencia de comandos](#referencia-de-comandos)
12. [Referencia de ajustes](#referencia-de-ajustes)
13. [Dónde están los archivos](#dónde-están-los-archivos)
...
Ambos usan el mismo Client y el mismo protocolo. Puedes empezar con el alojado y mudarte después, o incluso mantener los dos, porque un Tunnel es independiente de cualquier cuenta. Un Gateway también es independiente de un Tunnel: si alguien de confianza ya tiene uno en marcha, pon tu propio Tunnel encima y ahórrate el servidor por completo — véase [Compartir un Gateway](#shared-gateway).
...
Los Clientes de escritorio y de móvil hacen todo el recorrido, así que no hay ningún archivo que mover entre máquinas:

1. **Instala el Cliente** — desde [lantunnel.app/download](https://lantunnel.app/download) o compilándolo desde este repositorio.
2. **Inicia sesión** — pulsa «Sign in» en la pantalla de conexión. El Cliente muestra un código corto y abre tu navegador. Comprueba que el código de la página coincide con el del Cliente, apruébalo y el Cliente inicia sesión solo. Aprueba únicamente un código que estés leyendo en ese momento en tu propio dispositivo.
3. **Añade un Peer** — pulsa «Add a Peer» y elige el Tunnel por su nombre. Después toma un Peer que el Tunnel ya haya emitido —un dispositivo añadido antes desde la Consola ya tiene uno— o ponle nombre a este dispositivo y crea uno nuevo. El perfil se importa en el mismo paso.
4. **Conecta.**

Repítelo en cada dispositivo. Un Tunnel o un Peer creado hace un momento en el navegador aparece al pulsar «Refresh» junto al selector de Tunnel.

Iniciar sesión guarda en el dispositivo un token de esa cuenta. Puede listar tus Tunnels y añadirles Peers, así que se guarda solo para el propietario —permisos `0600` en Linux y macOS, el Llavero en iOS, almacenamiento privado de la app en Android— y «Sign out» lo elimina. Caduca por sí solo, y uno caducado cierra la sesión del Cliente en vez de renovarse.

### Desde el navegador

No cambia, y sigue siendo el camino de un host sin interfaz, que no tiene pantalla donde mostrar un código:
...
Para añadir otro Tunnel más adelante basta con dejar otro `.scope` en `scopes_dir`: un Gateway admite tantos Tunnels como scopes tenga, y cada uno puede ser de otra persona. En Unix, `kill -HUP <pid>` — con la unidad incluida, `systemctl reload lantunnel-gateway` — relee el directorio sin tocar las conexiones vivas. Quitar un `.scope` y recargar desconecta a los Peers de ese Tunnel. Una recarga que no pase la validación se rechaza y sigue vigente el último conjunto correcto. Hay unidades de systemd de ejemplo en [`scripts/remote/`](../scripts/remote/).
...
### Compartir un Gateway entre dos dueños

Un Gateway no está atado a un solo Tunnel, así que la máquina de otra persona puede llevar tu Tunnel sin que ninguno de los dos entregue nada privado.

**Si el Gateway es tuyo:** reparte el transporte, la dirección, el puerto de datos, el puerto de mapping y, con certificado autofirmado, el `certs/server.crt` público. Nunca `certs/server.key`. Deja cada `.scope` que te envíen en `scopes_dir` y recarga. Estás admitiendo ese Tunnel, no entrando en él: no puedes emitir un Peer dentro y solo ves bytes sellados.

**Si el Tunnel lo pones tú:** ejecuta los pasos 3, 4 y 6 con los datos de conexión de la otra persona y sáltate el 1, el 2 y el 5. Envía solo el `<tunnel-id>.scope` público y guarda el `.tunnel` — el archivo que firma las membresías — en tu propia máquina, de modo que nadie salvo tú pueda añadir un Peer a tu Tunnel. Toda su palanca es la admisión: si borra tu `.scope` y recarga, el Gateway deja de aceptar tus Peers.
...
Descarga `lantunnel-client-headless-<version>-<triple>` en lugar del paquete de escritorio. Es el mismo runtime con la interfaz eliminada en compilación: sin Tauri, sin WebView, 5–7MB en vez de un AppImage de ~97MB, y sin necesidad de bibliotecas gráficas en el host. Las versiones de Linux se enlazan estáticamente contra musl. Cualquier arranque de ese binario es headless, así que `--headless` está implícito y la opción solo sigue ahí para guiones compartidos con equipos de escritorio.

Hay una capacidad que no viene incluida: instalar el ayudante TUN privilegiado de macOS es una acción de la app de escritorio, así que un host macOS headless solo puede usar enrutamiento nativo si el Client de escritorio instaló antes ese ayudante. Linux y Windows no se ven afectados —resuelven el sidecar TUN igual que la versión de escritorio— y un Peer exportador no necesita TUN en absoluto.
...
**En Windows**, la compilación de escritorio usa el subsistema GUI, así que un arranque normal no abre ninguna consola y `cmd.exe` no espera al proceso. Cuando te importen la salida y el código de salida de un comando corto, usa `start /wait`. La compilación headless mantiene el subsistema de consola y no necesita nada de esto:
...
## En un router (OpenWrt)

El router ya está encendido todo el día y ya está en la LAN, así que es el sitio natural para el Peer que [exporta esa LAN](#compartir-una-lan-entera). Los equipos que hay detrás no necesitan instalar nada.

Exportar una subred solo marca hacia fuera, así que no hace falta un dispositivo TUN, ni `kmod-tun`, ni `ip-full`, ni ningún cambio de rutas o cortafuegos.

Elige el tarball que corresponda a `uname -m` — `aarch64`, `armv7l` o `x86_64` — y descomprímelo en la raíz del sistema de archivos:

```sh
tar -xzf lantunnel-client-openwrt-<version>-<arch>.tar.gz -C /
```

Eso instala `/usr/bin/lantunnel-client`, un servicio procd en `/etc/init.d/lantunnel` y `/etc/config/lantunnel`. Importa el perfil y arranca el servicio:
...
Con un único perfil importado, el servicio se conecta a ese. Con más de uno, nómbralo con
`uci set lantunnel.main.tunnel_id=<TUNNEL_ID> && uci commit lantunnel`.

La configuración vive en el overlay de flash, en `/etc/lantunnel`; los registros van a `/var/log/lantunnel`, que es tmpfs, para que un fichero de log rotado a diario nunca toque la flash. `/etc/config` sobrevive a un sysupgrade automáticamente; para conservar también el perfil importado, añade `/etc/lantunnel` a `/etc/sysupgrade.conf`. El binario nunca se conserva: reinstálalo tras la actualización.

Instalado, el binario ocupa 5,5MB en armv7, 5,7MB en aarch64 y 7,1MB en x86_64. Eso descarta los routers con 16MB de flash, que son la mayoría del hardware `ath79` y `ramips`, y MIPS no se compila en absoluto.

Las instrucciones completas de instalación viajan dentro del tarball, en `/usr/share/lantunnel/README.md`.
...
Sustituciones por entorno: `LANTUNNEL_LOCAL_SOCKS5_LISTEN`, `LANTUNNEL_DESKTOP_NETWORK_MODE`, `LANTUNNEL_LAN_ROUTES`, `TUNNEL_PROXY_APP_CONFIG_DIR`, `LANTUNNEL_LOG_DIR`.
...
| Registros del Client | junto a la configuración (se puede cambiar con `LANTUNNEL_LOG_DIR`; así es como el paquete del router los mantiene fuera de la flash) |

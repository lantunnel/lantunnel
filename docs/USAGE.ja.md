...
デスクトップとスマートフォンの Client は最後まで自力でできるので、マシン間でファイルを運ぶ必要がありません。

1. **Client を入れる** — [lantunnel.app/download](https://lantunnel.app/download) から、またはこのリポジトリからビルドして。
2. **サインイン** — 接続画面の「Sign in」を押します。Client が短いコードを表示してブラウザーを開きます。ページのコードと Client のコードが同じことを確かめて承認すれば、Client は自分でサインインします。承認してよいのは、いま自分のデバイスで読み取っているコードだけです。
3. **Peer を追加** — 「Add a Peer」を押し、Tunnel を名前で選びます。その Tunnel がすでに発行した Peer を引き取るか（Console から先に追加した端末にはすでに 1 つあります）、この端末に名前を付けて新規作成するかを選べます。プロファイルは同じ操作で取り込まれます。
4. **接続。**

端末ごとに繰り返します。ブラウザーで作ったばかりの Tunnel や Peer は、Tunnel 選択欄の「Refresh」を押すと現れます。

サインインすると、そのアカウントのトークンが端末に保存されます。Tunnel 一覧を取得し Peer を追加できるものなので、所有者だけが読める形で保管されます — Linux と macOS では `0600`、iOS では Keychain、Android ではアプリ専用ストレージ。「Sign out」で削除されます。有効期限があり、切れた場合は自動更新ではなくサインアウトします。

### ブラウザーで済ませる

こちらは変わりません。画面がなくコードを表示できないヘッドレスホストでは、今もこの手順だけです。
